...
public class ValidationConfig {
    ...
    public ValidationHandler bookingValidationChain(DoctorRepository doctorRepository,
                                                    ...
                                                    @Value("${medsys.rules.max-per-day:3}") int maxPerDay) {
        ...
        ValidationHandler biz  = new BusinessRulesCheck(appointmentRepository, maxPerDay); // <— pass it
        ...
        log.info("Booking validation CoR initialized: Auth -> Role -> DoctorAvailability -> PatientConflict -> BusinessRules (limit={})", maxPerDay);
        return auth;
    }
}
